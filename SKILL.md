...
**第三步（MITM 场景专属）：澄清修改目标** — 若用户需求属于 MITM 场景且涉及请求包修改，**必须询问用户**：「您是仅需修改自己在 MITM 中查看的请求包（仅影响本地展示），还是需要修改真正发往服务器的请求包（影响实际请求）？」根据用户回答选择函数：
- 用户选择「仅修改查看的（解密推荐）」 → 使用 `hijackSaveHTTPFlow`，在流量入库前处理，仅改变本地 History 中的展示，不影响实际发往服务器的请求
- 用户选择「修改发往服务器的」 → 使用 `hijackHTTPRequest`，在请求转发前修改，真正改变发往服务器的数据包
**第四步：输出代码** — 基于知识生成可直接粘贴到 Yakit 热加载编辑器中运行的完整代码。
...
1. **函数选择**：根据场景选择正确的函数
   - MITM 修改请求 → 先询问用户是「仅修改查看」还是「修改发往服务器的请求」
     - 仅修改查看的（仅影响本地 History 展示） → `hijackSaveHTTPFlow`
     - 修改发往服务器的（影响实际请求） → `hijackHTTPRequest`
